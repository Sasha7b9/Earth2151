...
bool TexLand2150::BuildCanvas(FileTex2150 *tex)
{
    ...
    {
        LOG_ERROR("Error texture file : %d textures instead of 512", tex->textures.GetArrayElementCount());
    ...
    for (int counter = 0; counter < 512; counter++)
    {
        ...
        if (color->alpha != 1.0f)
        {
            LOG_ERROR("In texture pixel alpha non zero");
        }
...
bool TexLand2150::GetTile(const Tile tile, TexTile &tex)
{
    ...
    if (!result)
    {
        ...
        {
            first = false;
            LOG_ERROR("Not tile for cell %d %d %d %d", tile.tex[0], tile.tex[1], tile.tex[2], tile.tex[3]);
        }
...
void TexLand2150::CreateTiles()
{
    ...
    {
        LOG_ERROR("Can not open file %s", file.GetFileName());
    }

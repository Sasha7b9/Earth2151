...
struct Building : public EquipableEntity
{
        ...
        EquipableEntity(name, researhces, type, reader)
    {
        ...
        init_cannon_ID1 = reader.ReadString();

        reader.ReadUInt();
        init_cannon_ID2 = reader.ReadString();

        reader.ReadUInt();
        init_cannon_ID3 = reader.ReadString();

        reader.ReadUInt();
        init_cannon_ID4 = reader.ReadString();

        reader.ReadUInt();
        copula_ID = reader.ReadString();

        reader.ReadUInt();
        building_tunnel_number = reader.ReadInt();
        upgrade_copula_small_ID = reader.ReadString();

        reader.ReadUInt();
        upgrade_copula_big_ID = reader.ReadString();

        reader.ReadUInt();
        build_LC_transporter_ID = reader.ReadString();

        reader.ReadUInt();
        chimmey_smoke_ID = reader.ReadString();

        reader.ReadUInt();
        need_power = reader.ReadInt();
        slave_building_ID = reader.ReadString();

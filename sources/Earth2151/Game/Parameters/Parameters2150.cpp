...
Parameters2150::Parameters2150(pchar f) :
    file_name(f)
{
    File file;
    ...
    {
        LOG_ERROR_HI("Can not open file \"%s\"", file_name.c_str());
    ...
    {
        LOG_ERROR_HI("Wrong parameter file \"%s\"", file_name.c_str());
...
bool Parameters2150::ReadGroups(FileReader &reader)
{
    ...
    for (uint i = 0; i < group_count; i++)
    {
        ...
        {
            LOG_ERROR_HI("Dont read group %d from %d", i + 1, group_count);
            return false;
        }
    }
...
void Parameters2150::Save()
{
    ...
    {
        LOG_ERROR_HI("Can not create file %s", file_txt.c_str());
        return;
    }

...
ObjectViewportWidget::ObjectViewportWidget(const Vector2D& size, float focalLength) : Canvas(size)
{
    ...
    viewport->LoadViewportWorld("world/test");
    viewport->EnableCameraOrbit(Point3D::zero, 1.0f, 1.0f);
    viewport->SetCameraAngles(0.0f, 0.5f);
//    viewport->GetViewportWorld()->GetRootNode()->AppendNewSubnode(sunViewport);
    viewport->SetWidgetPosition({ 1.0f, 1.0f });
